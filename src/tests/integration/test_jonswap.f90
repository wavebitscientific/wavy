...
write(*,*)
write(*,*)'Test omnidirectional spectrum with JONSWAP shape;'
...
do m = 1,size(fetch)
  ...
  do n = 1,size(wspd)
    ...
    ! Set the spectrum field to JONSWAP parametric shape
    spec = jonswap(spec % getFrequency(),wspd=wspd(n),&
                   fetch=fetch(m),grav=grav)
